...
1. SQL Editor で `migrations/20260813130000_srs_cards.sql` を Run  
   （`srs_cards` / RLS / 例題削除時 CASCADE）
2. アプリ
   - キーワード自己説明が **A / B** → 復習カード登録（裏面は前回の自己説明。再登録時は裏面だけ更新）
   - 例題を **正解**（機能リリース後）→ 復習カード登録（既存カードのスケジュールは維持）
   - メイン上部「今日の復習」: due 件数バッジ + 最大20枚の混合セッション
   - 評価ボタン: 今すぐ / 難しい / 普通 / 簡単（Anki SM-2 系）
...
3. 仕様メモ
   ...
   - `examples.user_id IS NULL` = 共有バンク（画面上は「試験問題」）
   - 例題リスト: 自分の例題 ＋ `user_example_library` で取り込んだ共有問題
   - 共有問題の「削除」= 取り込み解除（再取り込み可）
   - `domain`: 資格ごとのブループリント `domain_key`（SAP 固定 CHECK は撤廃済み）

## 試験問題バンク（資格ブループリント）

1. SQL Editor で次を Run  
   - `migrations/20260814120000_exam_bank_status.sql`（`exam_bank_status` / `exam_bank_message`）  
   - `migrations/20260822130000_exam_blueprints.sql`（`exam_question_count` / `exam_blueprints` / `exam_blueprint_domains` / SAP シード）
2. Edge Function をデプロイ
   ```bash
   npx supabase functions deploy generate-exam-bank
   npx supabase functions deploy generate-example
   npx supabase functions deploy validate-certification
   ```
3. 仕様メモ
   - バンク目標件数 = `exam_question_count × 2`（例: 本番75問 → バンク150問）
   - ドメイン比率・複数選択目安は `exam_blueprints` / `exam_blueprint_domains`（管理者シード。編集 UI は後続）
   - ドメイン未設定の資格は「総合」1ドメインで目標件数まで生成
   - `exam_question_count` 未設定の資格は生成不可（資格追加時に出題数を登録）
   - メイン「本番試験を受ける」→ 試験問題が無ければ作成確認
   - 1呼び出しあたり最大5問。途中失敗時も既存は残し、再実行で再開
   - 画面文言は「試験問題」（内部概念は共有バンクのまま）
